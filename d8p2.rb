data = DATA.readlines.map { |line| line.split(" | ").map { |section| section.split(" ") } }
output = []
data.each { |entry|
  patterns, values = entry
  patterns.map! { |word| word.chars.sort }
  digits = Array.new(10)

  # unambiguous
  digits[1] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 2 }))
  digits[4] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 4 }))
  digits[7] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 3 }))
  digits[8] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 7 }))

  # six-digit #s
  # 9 contains 4
  digits[9] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 6 && (digits[4] - p).none? }))
  # 0 contains 1
  digits[0] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 6 && (digits[1] - p).none? }))
  # else 6
  digits[6] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 6 }))

  # five-digit #s
  # 3 contains 7
  digits[3] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 5 && (digits[7] - p).none? }))
  # 5 is contained by 6
  digits[5] = patterns.delete_at(patterns.index(patterns.find { |p| p.size == 5 && (p - digits[6]).none? }))
  # else 2
  digits[2] = patterns.first

  digits.map!(&:join)
  values.map! { |word| word.chars.sort.join }
  output << values.map { |word| digits.index(word) }.map(&:to_s).join.to_i
}

puts output.flatten.sum
